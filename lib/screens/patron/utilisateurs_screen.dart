...
class _UtilisateursScreenState extends State<UtilisateursScreen> {
  ...
  Future<void> _charger() async {
    ...
    if (id == null) return;
    try {
      final users = await _service.listUsers(id);
      if (mounted) {
        setState(() {
          _users = users;
          _chargement = false;
  ...
  Future<void> _ajouter() async {
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<RoleUtilisateur>(
                  ...
                  items: const [
                    DropdownMenuItem(
                      value: RoleUtilisateur.patron,
                      child: Text('Patron (admin du pressing)'),
                    ),
                    DropdownMenuItem(
                      value: RoleUtilisateur.employe,
    ...
    if (ok == true) {
      ...
      try {
        await _service.createUserViaSignUp(
  ...
  String _libelleRole(RoleUtilisateur role) =>
      role == RoleUtilisateur.patron ? 'Patron (admin)' : 'Utilisateur simple';

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Utilisateurs du pressing')),
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  child: ListTile(
                    ...
                    subtitle: Text(
                      '${_libelleRole(u.role)} · ${u.actif ? 'Actif' : 'Désactivé'}',
                    ),
                    trailing: u.actif
                        ? IconButton(
                            ...
                            tooltip: 'Désactiver',
                            onPressed: () async {
                              await _service.deactivateUser(u.id);
                              _charger();
                            },
                          )
                        : null,
